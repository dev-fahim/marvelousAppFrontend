import { LoanGivingRecieveService } from './../../../../service/loan-giving/loan-giving-recieve.service';
...
@Component({
  selector: 'app-loan-giving-debit-list',
  templateUrl: './loan-giving-debit-list.component.html',
  styleUrls: ['./loan-giving-debit-list.component.scss']
})
export class LoanGivingDebitListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private _loanGivingRecieveService: LoanGivingRecieveService,
  ...
  ngOnInit() {
    ...
    this._loanGivingRecieveService.get_all_expenditures({
      ...
      .subscribe(
        (result: any) => {
          ...
          for (let expend of result) {
            if (expend.is_deleted === false && expend.is_for_refund === false && expend.is_for_return === true) {
              data.push(expend);
            }
          }
  ...
  onFilter(filtered_data = {}) {
    this._loanGivingRecieveService.get_all_expenditures(filtered_data)
      .subscribe(
        (result) => {
          this.loading = false;
          const data = [];
