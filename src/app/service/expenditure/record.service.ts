...
export class RecordService {
  ...
  public baseUrl = EXPENDITURE_RECORD_REST_API_URL;

  constructor(private _http: HttpClient, private _router: Router) { }

  get_all_expenditures(filters: ExpenditureRecordFilter) {
    return this._http.get<models.ExpenditureRecordGETModel[]>(this.baseUrl + 'list/', {
      params: {
  ...
  add_record(data: models.ExpenditureRecordPOSTModel) {
    return this._http.post(this.baseUrl + 'add/', JSON.stringify(data)).pipe(
      catchError(
        (error: HttpErrorResponse) => {
          return throwError(common.get_http_response_error(error))
        }
  ...
  get_specific_record(uuid: string) {
    return this._http.get<models.ExpenditureRecordGETModel>(this.baseUrl + 'view/' + uuid + '/').pipe(
      catchError(
        (error: HttpErrorResponse) => {
          return throwError(common.get_http_response_error(error))
        }
  ...
  update_record(data: models.ExpenditureRecordPUTModel, uuid: string) {
    return this._http.put<models.ExpenditureRecordPUTModel>(
      this.baseUrl + 'view-update-delete/' + uuid + '/', data
    ).pipe(
      catchError(
        (error: HttpErrorResponse) => {
          return throwError(common.get_http_response_error(error))
  ...
  delete_record(uuid: string, payloads: models.ExpenditureRecordPUTModel) {
    return this._http.put<models.ExpenditureRecordPUTModel>(this.baseUrl + 'view-update-delete/' + uuid + '/', payloads).pipe(
      catchError(
        (error: HttpErrorResponse) => {
          return throwError(common.get_http_response_error(error))
        }
